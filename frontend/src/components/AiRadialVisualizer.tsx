...
export default function AiRadialVisualizer({ player, size = 220, className = "" }: AiRadialVisualizerProps) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const amRef = useRef<any>(null);

  useEffect(() => {
    let destroyed = false;
    let localAnalyzer: any = null;
    ...
    const setup = async () => {
      ...
      localAnalyzer = new AudioMotionAnalyzer(containerRef.current, {
        ...
        spinSpeed: 0.6, // subtle spin for 3D-like motion
        // Bars/graph look
      ...
      try {
        localAnalyzer.connectInput(outputNode as any);
      } catch (_) {
